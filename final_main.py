...
class ImageViewer(Frame):
    def __init__(self, master, TrainingImageDirectory, TrainingMaskDirectory, UnetWeights50, RawDataPath, ImgSize=256):
        ...
        self.ButtonBack = Button(master, command=self.ButtonBackClick, text="Back", width=14, height=2)
        self.ButtonExit = Button(master, command=master.quit, text="Exit", width=14, height=2)
        self.ButtonForward = Button(master, command=self.ButtonForwardClick, text="Forward", width=14, height=2)
        self.ButtonPainter = Button(master, command=self.CallPainter, text="Modify label", width=14, height=2)

        self.ButtonBack.grid(row=2, column=1)
        self.ButtonExit.grid(row=2, column=9)
        self.ButtonForward.grid(row=2, column=2)
        self.ButtonPainter.grid(row=2,column=3)

        self.canvas1 = Canvas(master, width=250, height=250)
        self.canvas1.grid(row=1, column=0, columnspan=3, ipadx=0, padx=0)
        self.canvas2 = Canvas(master, width=250, height=250)
        self.canvas2.grid(row=1, column=3, columnspan=3, ipadx=0, padx=0)
        self.canvas3 = Canvas(master, width=250, height=250)
        self.canvas3.grid(row=1, column=7, columnspan=3, ipadx=0, padx=0)
    ...
    def LoadRawData(self, DataPath): #Loads the raw images (presumably not annotated ones)
        DataPath.replace("/", "\\")
        self.ImageNames = os.listdir(DataPath)
        if np.shape(self.ImageNames)[0]==0:
            logging.warning("There are no files in the directory")
        for ImageName in self.ImageNames:
            print(ImageName)
            try:
                grayImage = cv2.imread(DataPath + ImageName, 0)
                grayImage = Image.fromarray(grayImage)
                if np.shape(grayImage)!=[self.ImgSize, self.ImgSize]:
                    logging.info('The image got reshaped to the standard image size of the app')
                grayImage = grayImage.resize((self.ImgSize, self.ImgSize))
                self.RawImages.append(np.array(grayImage))
            except:
                logging.error("Something went wrong during the reading of " + ImageName)

    def __ApplyTransformation(self, img): #Applies the mithocondria segmenter U-net defined in the previous class to the given image
        Result =  self.MSegmenter.PredictResult(normalize(img))*255
        rgbResult = cv2.cvtColor(Result, cv2.COLOR_GRAY2RGB)
        return rgbResult

    def __MaskOnImage(self, img, mask): #Generates the masked image as an output from the original image and the mask. The mask will be colored purple.
    ...
    def __IncrementImgNum(self): #Increments the number (ID) of the currently displayed and modifyable image
        self.ImageCounter+=1
        if(len(self.RawImages)<=self.ImageCounter): self.ImageCounter=0

    def __DecrementImgNum(self): #Decrements the number (ID) of the currently displayed and modifyable image
        self.ImageCounter-=1
        if (0 > self.ImageCounter): self.ImageCounter = len(self.RawImages)-1

    def ButtonBackClick(self): #Calls the Decrement function and then calculates and displayes the respective images belonging to the given image ID
    ...
    def ButtonForwardClick(self): #Calls the Increment function and then calculates and displayes the respective images belonging to the given image ID
    ...
    def CallPainter(self): #Calls the Painter App in a separate window with the currently handled images defined by the image ID
...
class PaintApplication:
    ...
    def render_label(self): #Displayes the masked image to the editing tool
